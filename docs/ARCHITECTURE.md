...
Effective native and ordinary contexts omit an interrupted/error assistant tool-call block only when its own fenced v1 transport evidence positively identifies an incomplete, nonnative item. Full selected-branch results, admission evidence, identity aliases, malformed reports, and conflicting attempts veto exclusion. Canonical entries and signature indexes remain unchanged; the failed assistant envelope and all other blocks survive. `UNADMITTED_CALL_OMITTED` is an informational projection diagnostic, not a tool result or recovery admission.

Provider error frames may add `nativeProviderError` with version 1 and fixed allowlisted code/category tokens. Unknown values map to `unknown`; raw messages, parameters, and payloads are not retained. This optional assistant metadata does not change retry classification or `errorMessage` and is reset between transport attempts.
...
Harness bookkeeping appends await the coordinator's active compaction barrier. Incoming family history, usage receipts, progress, and child-creation records therefore cannot invalidate an in-flight summary. The barrier releases on success, failure, or cancellation; existing message delivery and acknowledgement ordering remains intact. The strict snapshot validation remains in place. Automatic compaction failures use the normal canonical assistant-error commit path so a failed turn remains visible after detachment or restart.
