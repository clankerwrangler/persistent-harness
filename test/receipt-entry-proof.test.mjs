...
for (const userLabel of ["user", "Operator"]) for (const source of ["cron", "background"]) test(`${source} ${userLabel} headers retain receipt authority and reject metadata/formatter conflicts`, async (t) => {
  ...
  assert.match(entry.content[0].text, /not a new user submission/);
  entry.content[0].text = entry.content[0].text.replace("not a new user submission", `not a new ${userLabel} submission`);
  for (const completed of [false, true]) {
    if (completed) { f.store.completeActorInput(input.inputId, "s", 20, entry.id); f.reopen(); }
    const visible = await f.read([entry]); assertLedger(visible, [[input.inputId, entry]]);
...
test("the actor reporter skips literal candidates and reaches later real entries, including the same candidate ID", async (t) => {
  ...
  entries.push(user("literal-first", input.inputId, "user quotes another body"));
