...
async function fixture(t, { holdReconnect = false, dropAck = false, rejectFlush = false, dropFlush = false, timeoutMs = 600, historyGate } = {}) {
  ...
  const hooks = new Map(), owner = {}, historyWrites = []; let client, heldRegistration;
  ...
  if (historyGate) owner.appendHistoryEntry = async (type, data) => {
    historyWrites.push({ type, data }); await historyGate.promise;
    return ctx.sessionManager.appendCustomEntry(type, data);
  };
  ...
  return { client, ctx, hooks, controller, store, sent, requests, registrations, acknowledgements, sockets, notices, entries, canonicalEntries, historyWrites,
...
test("incoming family history waits for the compaction owner before persistence and injection", { timeout: 5000 }, async t => {
  const historyGate = Promise.withResolvers(); t.after(() => historyGate.resolve());
  const f = await fixture(t, { historyGate });
  const delivered = f.family();
  await waitFor(() => f.historyWrites.length === 1, "incoming history reaches owner");
  assert.deepEqual(f.entries, [], "no bookkeeping append during compaction");
  assert.equal(f.sent.length, 0, "injection must still follow the history write");
  assert.equal(f.requests.filter(r => r.type === "record_agent_message_entry").length, 0);
  historyGate.resolve(); await delivered;
  await f.hooks.get("agent_settled")();
  assert.equal(f.entries.filter(e => e.customType === "persistent-harness.agent-message-v1").length, 1);
