...
test("default root names stay cwd-basename plus short id", () => {
  ...
  assert.equal(shortSessionId(sessionId), "11e594f4");
  assert.equal(defaultRootName("/workspace", sessionId), "workspace-11e594f4");
  assert.equal(isDefaultRootName("workspace-11e594f4", "/workspace", sessionId), true);
  assert.equal(isDefaultRootName("Auto-Rename", "/workspace", sessionId), false);
});

test("first user prompt becomes a short title", () => {
  assert.equal(
  ...
  assert.equal(titleFromUserPrompt("SSH into server and check disk"), "SSH Into Server and Check Disk");
...
test("unique root titles append the short id only when needed", () => {
  ...
  assert.equal(allocateUniqueRootName("Fix the Login Bug", "11e594f4", (name) => taken.has(name)), "Fix the Login Bug 11e594f4");
  assert.equal(allocateUniqueRootName("Fresh Title", "11e594f4", (name) => taken.has(name)), "Fresh Title");
});
...
test("generated titles stay five words and reject sentences", () => {
  assert.equal(
    normalizeGeneratedTitle('Video Playback Keeps Pausing on My Device During a Stream'),
    null,
  );
...
test("fallback titles obey the same word and sentence rules instead of freezing clipped prompts", () => {
  for (const userText of [
    ...
    "Isn't the calendar hosted on Server",
    "user, you say it broke then it broke",
  ]) {
  ...
  assert.equal(normalizeGeneratedTitle("User Interaction Rules"), "User Interaction Rules");
