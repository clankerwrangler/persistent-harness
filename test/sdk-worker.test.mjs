...
test("external compaction hook receives canonical projections and composed namespace instructions", async () => {
  ...
  await writeFile(extensionPath, `export default function(pi) {
    pi.on("session_before_compact", event => {
      ...
      for (const mode of ["native", "ordinary"]) {
        ...
        if (request.result.outstanding.length || request.result.diagnostics.some(item => item.severity === "blocking"))
          return { cancel: true };
      ...
      return { compaction: { summary: "external hook summary", tokensBefore: event.preparation.tokensBefore,
        firstKeptEntryId: event.preparation.firstKeptEntryId,
        details: { projections, branchEntries: event.branchEntries, instructions: event.customInstructions } } };
    });
  }`);
  let projectionBus;
  const { worker } = await fixture({
  ...
  try {
    ...
    const interruptedWait = (name, prefixCallId, prefixItemId) => {
      const callId = `call_${name}`, itemId = `fc_${name}`;
      const attempt = { attemptId: `attempt_${name}`, attemptNumber: 1, transport: "websocket",
        sent: true, created: true, responseId: `response_${name}`, observedCalls: [
          { itemId: prefixItemId, callId: prefixCallId, complete: true, native: true },
          { itemId, callId, complete: false, native: false },
        ], providerTools: false, retired: true, fenced: true, possibleUsage: true, outcome: "unknown" };
      return assistant([{ type: "toolCall", id: `${callId}|${itemId}`, name: "wait_for_ipython", arguments: {} }],
        { id: `message_${name}`, stopReason: "error", errorMessage: "websocket_closed", responseId: attempt.responseId,
          nativeTransport: { version: 1, requestId: `request_${name}`, ...attempt, attempts: [structuredClone(attempt)] } });
    };
    ...
    const failedWait = interruptedWait("wait_fixture", "call_fixture", "fc_fixture");
    const failedEntry = manager.appendMessage(failedWait);
    ...
    const canonicalBefore = structuredClone(manager.getEntries());
    const expectedBranch = projectCanonicalBranch({ entries: canonicalBefore, leafId: manager.getLeafId() }).entries;
    assert.deepEqual(expectedBranch.find(entry => entry.id === failedEntry).message, failedWait);
    ...
    assert.deepEqual(result.details.branchEntries, expectedBranch, "hooks receive unfiltered canonical evidence with signature overlays");
    assert.deepEqual(manager.getEntries().slice(0, -1), canonicalBefore);
    assert.equal(manager.getEntries().length, canonicalBefore.length + 1, "only the normal compaction entry is appended");
    ...
    for (const projection of [native, ordinary]) {
      ...
      assert.deepEqual(projection.messages.find(message => message.id === failedWait.id), { ...failedWait, content: [] },
        "only the never-admitted block is omitted; error, usage, and transport evidence survive");
      assert.equal(projection.messages.some(message => message.role === "assistant"
        && message.content.some(block => block.type === "toolCall" && block.id === failedWait.content[0].id)), false);
    ...
    expectedOrdinary.splice(expectedOrdinary.findIndex(message => message.id === "message_fixture") + 1, 0,
      expectedNative.find(message => message.role === "toolResult"));
    assert.deepEqual(ordinary.messages, expectedOrdinary, "ordinary projection only omits the wait and moves the known native result");
    ...
    assert.equal(input.some(item => item.type === "function_call" && item.call_id === "call_wait_fixture"), false);
    assert.equal(input.some(item => item.type === "function_call_output" && item.call_id === "call_wait_fixture"), false);
    ...
    // A separate admitted prefix with no result must still block compaction.
    manager.appendMessage(user("unanswered native question ".repeat(100)));
    const pendingId = "call_pending|fc_pending";
    const pendingEntry = manager.appendMessage(assistant([
      { type: "toolCall", id: pendingId, name: "read", arguments: { path: "pending" }, async: true,
        providerCallId: "call_pending", providerItemId: "fc_pending" },
    ], { id: "message_pending", stopReason: "toolUse" }));
    const pendingWait = interruptedWait("wait_pending", "call_pending", "fc_pending");
    manager.appendMessage(pendingWait);
    const pendingBefore = await readFile(file, "utf8");
    for (const mode of ["native", "ordinary"]) {
      const request = { entries: structuredClone(manager.getBranch()), leafId: manager.getLeafId(), mode };
      projectionBus.emit(channel, request);
      assert.equal(request.error, undefined);
      assert.deepEqual(request.result.outstanding, [{ toolCallId: pendingId, toolName: "read",
        messageEntryId: pendingEntry, contentIndex: 0, retainedInContext: true }]);
      assert.deepEqual(request.result.messages.find(message => message.id === pendingWait.id), { ...pendingWait, content: [] });
      assert.equal(request.result.messages.some(message => message.role === "toolResult" && message.toolCallId === pendingId), false);
      if (mode === "ordinary") assert(request.result.diagnostics.some(item => item.code === "ORDINARY_REPLAY_BLOCKED"
        && item.severity === "blocking" && item.toolCallId === pendingId));
    }
    await assert.rejects(worker.handle({ type: "compact" }), { code: "ERR_COMPACTION_PENDING_CALLS" });
    assert.equal(await readFile(file, "utf8"), pendingBefore, "pending admission is neither hidden nor repaired by compaction");
