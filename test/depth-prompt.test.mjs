...
test("depth 0 puts the depth file first and removes the coding-assistant prime", () => {
  ...
  assert.ok(assembled.indexOf("You are an assistant.") < assembled.indexOf("Available tools:"));
...
test("depth 0 puts the user prefix first verbatim", () => {
  const assembled = assembleActorSystemPrompt({
    depth: 0,
    basePrompt: defaultBasePrompt,
    depthPrompt: "# Depth-scoped instructions\n\nYou are an assistant.",
  ...
  assert.equal(assembled.includes("# user prefix"), false);
...
test("depth 0 omits empty user prefixes and keeps whitespace-only text verbatim", () => {
...
test("deeper actors ignore user prefix even when one is supplied", () => {
  const assembled = assembleActorSystemPrompt({
    depth: 1,
    basePrompt: defaultBasePrompt,
    depthPrompt: "CHILD ROLE",
