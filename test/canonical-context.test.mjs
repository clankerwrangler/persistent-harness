...
// Producer-shaped interrupted remainder: a committed native prefix and its real
// result are separate records. The final transport report describes BOTH items.
function failedSuffix() {
  const native = { type: "toolCall", id: "call_prefix|fc_prefix", name: "python", arguments: { code: "fixture" },
    async: true, providerCallId: "call_prefix", providerItemId: "fc_prefix" };
  const wait = { type: "toolCall", id: "call_wait|fc_wait", name: "wait_for_ipython", arguments: {} };
  const attempt = { attemptId: "attempt", attemptNumber: 1, transport: "websocket", sent: true, created: true,
    observedCalls: [{ callId: "call_prefix", itemId: "fc_prefix", complete: true, native: true },
      { callId: "call_wait", itemId: "fc_wait", complete: false, native: false }],
    providerTools: false, retired: true, fenced: true, possibleUsage: true, outcome: "unknown", responseId: "response" };
  return chain([assistant("prefix", [native]), result(native.id),
    { ...assistant("remainder", [wait]), stopReason: "error", errorMessage: "websocket_closed",
      nativeTransport: { version: 1, requestId: "request", ...attempt, attempts: [structuredClone(attempt)] } }]);
}
function syncAttempt(message) {
  const { version, requestId, attempts, ...attempt } = message.nativeTransport;
  attempts[attempts.length - 1] = structuredClone(attempt);
}

test("failed incomplete suffix is omitted in both effective modes, not canonical history or admitted prefix", () => {
  for (const stopReason of ["error", "aborted"]) for (const async of [undefined, false]) {
    const entries = failedSuffix(), failed = entries[2].message;
    failed.stopReason = stopReason;
    if (async !== undefined) failed.content[0].async = async;
    const before = structuredClone(entries);
    for (const mode of ["native", "ordinary"]) {
      const view = project(entries, mode);
      assert.deepEqual(view.messages, [entries[0].message, entries[1].message, { ...failed, content: [] }]);
      assert.deepEqual(view.outstanding, []);
      assert.deepEqual(view.diagnostics, [{ code: "UNADMITTED_CALL_OMITTED", severity: "info",
        toolCallId: "call_wait|fc_wait", toolName: "wait_for_ipython", messageEntryId: "e2", contentIndex: 0, retainedInContext: true }]);
      assert.equal(synthetic(view.messages).length, 0);
      assert.deepEqual(planUnknownRecovery({ ...input(entries, mode), timestamp: 5 }), []);
    }
    assert.deepEqual(projectCanonicalBranch({ entries, leafId: "e2" }).entries, before);
    assert.deepEqual(entries, before);
  }
});

const noProof = {
  "missing report": m => { delete m.nativeTransport; },
  "unknown version": m => { m.nativeTransport.version = 2; },
  "missing attempts": m => { delete m.nativeTransport.attempts; },
  "empty attempts": m => { m.nativeTransport.attempts = []; },
  "malformed attempt": m => { m.nativeTransport.attempts = [null]; },
  "top/latest mismatch": m => { m.nativeTransport.attempts[0].created = false; },
  "missing request identity": m => { delete m.nativeTransport.requestId; },
  "bad attempt identity": m => { m.nativeTransport.attemptId = null; syncAttempt(m); },
  "wrong attempt number": m => { m.nativeTransport.attemptNumber = 2; syncAttempt(m); },
  "unsupported transport": m => { m.nativeTransport.transport = "other"; syncAttempt(m); },
  "false-like fence": m => { m.nativeTransport.fenced = "true"; syncAttempt(m); },
  "missing provider tool flag": m => { delete m.nativeTransport.providerTools; syncAttempt(m); },
  "missing usage flag": m => { delete m.nativeTransport.possibleUsage; syncAttempt(m); },
  "bad response identity": m => { m.nativeTransport.responseId = {}; syncAttempt(m); },
  "native incomplete contradiction": m => { m.nativeTransport.observedCalls[1].native = true; syncAttempt(m); },
  "async admission": m => { m.content[0].async = true; },
  "provenance present": m => { m.content[0].nativeProvenance = {}; },
  "complete ordinary": m => { m.nativeTransport.observedCalls[1].complete = true; syncAttempt(m); },
  "native true": m => { Object.assign(m.nativeTransport.observedCalls[1], { complete: true, native: true }); syncAttempt(m); },
  "unfenced": m => { m.nativeTransport.fenced = false; syncAttempt(m); },
  "not retired": m => { m.nativeTransport.retired = false; syncAttempt(m); },
  "not sent": m => { m.nativeTransport.sent = false; syncAttempt(m); },
  "not created": m => { m.nativeTransport.created = false; syncAttempt(m); },
  "completed report": m => { m.nativeTransport.outcome = "completed"; syncAttempt(m); },
  "missing observations": m => { delete m.nativeTransport.observedCalls; syncAttempt(m); },
  "null observations": m => { m.nativeTransport.observedCalls = null; syncAttempt(m); },
  "nonarray observations": m => { m.nativeTransport.observedCalls = {}; syncAttempt(m); },
  "empty observations": m => { m.nativeTransport.observedCalls = []; syncAttempt(m); },
  "duplicate observation": m => { m.nativeTransport.observedCalls.push(structuredClone(m.nativeTransport.observedCalls[1])); syncAttempt(m); },
  "observation alias collision": m => { m.nativeTransport.observedCalls.push({ callId: "call_wait", itemId: "different", complete: false, native: false }); syncAttempt(m); },
  "malformed unrelated prefix": m => { m.nativeTransport.observedCalls[0].native = "true"; syncAttempt(m); },
  "wrong item": m => { m.nativeTransport.observedCalls[1].itemId = "different"; syncAttempt(m); },
  "wrong call": m => { m.nativeTransport.observedCalls[1].callId = "different"; syncAttempt(m); },
  "explicit partial identity": m => { m.content[0].providerCallId = "call_wait"; },
  "explicit conflicting identity": m => { Object.assign(m.content[0], { providerCallId: "other", providerItemId: "fc_wait" }); },
  "ambiguous composite": m => { m.content[0].id += "|extra"; },
  "plain identity": m => { m.content[0].id = "call_wait"; },
};
for (const key of ["complete", "native"]) for (const value of [undefined, null, 0, "false"]) {
  noProof[`${key} must be boolean ${String(value)}`] = m => {
    m.nativeTransport.observedCalls[1][key] = value; syncAttempt(m);
  };
}
for (const [name, mutate] of Object.entries(noProof)) test(`incomplete omission needs positive proof: ${name}`, () => {
  const entries = failedSuffix(); mutate(entries[2].message);
  const before = structuredClone(entries);
  for (const mode of ["native", "ordinary"]) {
    const view = project(entries, mode);
    ...
    if (mode === "ordinary") assert(view.diagnostics.some(d => d.code === "ORDINARY_ASSISTANT_NOT_REPLAYABLE"));
  }
  assert.deepEqual(entries, before);
});

test("successful turns and earlier conflicting attempt evidence cannot be excluded", () => {
  for (const stopReason of ["stop", "toolUse", "length"]) {
    const entries = failedSuffix(); entries[2].message.stopReason = stopReason;
    assert.deepEqual(project(entries).messages, entries.map(e => e.message));
  }
  for (const earlierNative of [false, true]) for (const latestNative of [false, true]) {
    const entries = failedSuffix(), m = entries[2].message;
    const first = structuredClone(m.nativeTransport.attempts[0]);
    first.observedCalls[1].complete = earlierNative; first.observedCalls[1].native = earlierNative;
    m.nativeTransport.attemptId = "attempt2"; m.nativeTransport.attemptNumber = 2;
    m.nativeTransport.observedCalls[1].complete = latestNative; m.nativeTransport.observedCalls[1].native = latestNative;
    const { version, requestId, attempts, ...latest } = m.nativeTransport;
    m.nativeTransport.attempts = [first, structuredClone(latest)];
    assert.equal(project(entries).diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), !earlierNative && !latestNative);
  }
});

test("canonical results and aliases across compaction cuts veto omission, sibling records do not", () => {
  for (const kind of ["result", "unknown-result", "call-alias", "item-alias", "orphan-provider-result"]) for (const pruned of [false, true]) {
    const entries = failedSuffix();
    let conflict;
    if (kind === "call-alias" || kind === "item-alias") {
      const id = kind === "call-alias" ? "call_wait|fc_other" : "call_other|fc_wait";
      conflict = assistant("conflict", [{ type: "toolCall", id, name: "python", arguments: {}, async: true }]);
    } else {
      conflict = { ...result(kind === "orphan-provider-result" ? "call_wait" : "call_wait|fc_wait"), toolName: "wait_for_ipython" };
      if (kind === "unknown-result") Object.assign(conflict, { isError: true, details: { nativeAsyncRecovery: "interrupted-unknown" } });
    }
    entries.push(messageEntry("conflict", "e2", conflict));
    if (pruned) {
      entries.push(messageEntry("kept", "conflict", user()));
      entries.push({ type: "compaction", id: "compact", parentId: "kept", timestamp, summary: "summary",
        tokensBefore: 10, firstKeptEntryId: "kept" });
      // New same-tuple failed call would be a canonical duplicate; do not conceal
      // that by pruning/filtering first. The original candidate is now pruned.
    }
    const view = project(entries, "ordinary");
    assert.equal(view.diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), false);
    if (!pruned) assert(view.diagnostics.some(d => d.code === "ORDINARY_ASSISTANT_NOT_REPLAYABLE"));
    const siblings = failedSuffix(); siblings.push(messageEntry("sibling", "e1", conflict));
    assert(project(siblings, "ordinary", "e2").diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"));
  }
  // Pruned ORPHAN results still veto a retained candidate by provider identity.
  const entries = failedSuffix();
  const orphan = messageEntry("orphan", null, { ...result("call_wait"), toolName: "wait_for_ipython" });
  entries[0].parentId = orphan.id;
  const compact = { type: "compaction", id: "compact", parentId: "e2", timestamp, summary: "summary",
    tokensBefore: 10, firstKeptEntryId: "e2" };
  const view = project([orphan, ...entries, compact], "ordinary");
  assert.equal(view.diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), false);
  assert(view.diagnostics.some(d => d.code === "ORDINARY_ASSISTANT_NOT_REPLAYABLE"));
});

test("duplicate IDs and STOCK helper tampering remain rejected before omission", () => {
  const entries = failedSuffix();
...
test("mixed failed blocks preserve late signature indexes and outstanding admitted recovery", () => {
  const entries = failedSuffix(), failed = entries[2].message;
  const signature = JSON.stringify({ type: "reasoning", id: "rs", summary: [] });
  failed.content = [{ type: "text", text: "partial" }, failed.content[0], call("unanswered"),
    { type: "thinking", thinking: "partial", thinkingSignature: signature }];
  entries.push(amendment(entries[2], "amendment", "e2", { contentIndex: 3 }));
  const before = structuredClone(entries);
  for (const mode of ["native", "ordinary"]) {
    const view = project(entries, mode), remainder = view.messages.find(m => m.id === "remainder");
    assert.equal(remainder.content.length, 3);
    assert.equal(JSON.parse(remainder.content[2].thinkingSignature).encrypted_content, "fixture-encryption");
    assert.equal(view.diagnostics.find(d => d.code === "UNADMITTED_CALL_OMITTED").contentIndex, 1);
    assert.equal(view.outstanding.length, 1);
    assert.equal(view.outstanding[0].toolCallId, "unanswered");
    assert.equal(view.outstanding[0].contentIndex, 2);
    const recovery = planUnknownRecovery({ ...input(entries, mode), timestamp: 10 });
    assert.equal(recovery.length, 1); assert.equal(recovery[0].toolCallId, "unanswered");
    if (mode === "ordinary") assert(view.diagnostics.some(d => d.severity === "blocking"));
  }
  assert.deepEqual(entries, before);
});


test("peer empty error envelopes can veto by matching observed evidence, never grant omission", () => {
  for (const observed of [
    { callId: "call_wait", itemId: "fc_wait", complete: true, native: true },
    { callId: "call_wait", itemId: "fc_wait", complete: true, native: false },
    { callId: "call_wait", itemId: "fc_other", complete: false, native: false },
    { callId: "call_other", itemId: "fc_wait", complete: false, native: false },
    { callId: "call_wait", complete: false, native: false },
  ]) for (const snapshot of ["top", "attempt"]) {
    const entries = failedSuffix();
    const peer = { ...assistant("peer", []), stopReason: "error", nativeTransport: snapshot === "top"
      ? { observedCalls: [observed] } : { attempts: [{ observedCalls: [observed] }] } };
    entries.push(messageEntry("peer", "e2", peer));
    assert.equal(project(entries).diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), false);
    // Sibling-only observations are not on the selected canonical branch.
    assert(project(entries, "native", "e2").diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"));
  }
  const entries = failedSuffix(), report = entries[2].message.nativeTransport;
  delete entries[2].message.nativeTransport;
  entries.push(messageEntry("peer", "e2", { ...assistant("peer", []), stopReason: "error", nativeTransport: report }));
  assert.equal(project(entries).diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), false);
});


test("internally inconsistent earlier attempt metadata cannot prove omission", () => {
  for (const overrides of [{ sent: false }, { outcome: "not_sent" }, { outcome: "rejected" }]) {
    const entries = failedSuffix(), report = entries[2].message.nativeTransport;
    const earlier = { ...structuredClone(report.attempts[0]), ...overrides };
    report.attemptId = "latest"; report.attemptNumber = 2;
...
test("consistent explicit provider identity and valid pre-output retry retain the narrow proof", () => {
  const entries = failedSuffix(), m = entries[2].message;
  Object.assign(m.content[0], { providerCallId: "call_wait", providerItemId: "fc_wait" });
  const earlier = { attemptId: "rejected", attemptNumber: 1, transport: "websocket", sent: true,
    created: false, providerTools: false, retired: true, fenced: true, possibleUsage: false,
    outcome: "rejected", observedCalls: [] };
  m.nativeTransport.attemptId = "latest"; m.nativeTransport.attemptNumber = 2;
  const { version, requestId, attempts, ...latest } = m.nativeTransport;
  m.nativeTransport.attempts = [earlier, structuredClone(latest)];
  assert(project(entries).diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"));
  m.nativeTransport.attempts[0].attemptId = "latest";
  assert.equal(project(entries).diagnostics.some(d => d.code === "UNADMITTED_CALL_OMITTED"), false);
});
...
test("unsent attempts require not_sent outcome and no response identity", () => {
  for (const outcome of ["not_sent", "unknown", "rejected", "failed", "completed"]) for (const responseId of [undefined, "impossible_response"]) {
    const entries = failedSuffix(), report = entries[2].message.nativeTransport;
    const earlier = { attemptId: "unsent", attemptNumber: 1, transport: "websocket", sent: false, created: false,
      providerTools: false, retired: true, fenced: true, possibleUsage: false, observedCalls: [], outcome };
    if (responseId !== undefined) earlier.responseId = responseId;
    report.attemptId = "latest"; report.attemptNumber = 2;
    const { version, requestId, attempts, ...latest } = report;
    report.attempts = [earlier, latest];
