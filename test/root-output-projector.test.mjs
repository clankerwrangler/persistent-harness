...
test("text deltas are emitted early with stable message/content sequencing before terminal", () => {
  ...
  assert.deepEqual(terminal[0].event, { type: "assistant_terminal", messageIndex: 1, status: "complete", stopReason: "stop", text: "First sentence. Second",
    presentation: { schema: "harness.presentation.v1", body: "talk-01", face: "common" },
...
test("mid-turn activation counts pre-subscription visible deltas and defers the full message to terminal", () => {
  const projector = new RootOutputProjector({ generation: "generation-a" });
  const marker = "<!-- harness.presentation.v1 body=happy face=smile -->\n";
  ...
  assert.deepEqual(terminal[0].event.presentation, { schema: "harness.presentation.v1", body: "happy", face: "smile" });
});
...
test("marker-only pre-subscription parsing keeps the first visible live delta cue-bearing", () => {
  ...
  assert.deepEqual(live[0].event.presentation, { schema: "harness.presentation.v1", body: "happy", face: "smile" });
});
...
test("presentation metadata precedes visible text, appears only on first sanitized delta, and repeats on terminal", () => {
  ...
  accept(projector, root("A"), 1, 1, { type: "agent_start" });
  accept(projector, root("A"), 1, 2, { type: "message_start", message: { role: "assistant" } });
  const marker = "<!-- harness.presentation.v1 body=encourage face=smile -->\n";
  let output = [];
  for (let index = 0; index < marker.length; index += 1) {
  ...
  assert.deepEqual(first[0].event.presentation, { schema: "harness.presentation.v1", body: "encourage", face: "smile" });
  ...
  assert.doesNotMatch(JSON.stringify(projector.subscribe({ generation: "generation-a", afterSeq: 0 }).events), /harness\.presentation\.v1 body=/);
});
...
test("toolUse turns carry independent per-assistant-message cues and indices", () => {
  ...
  const first = run(2, "<!-- harness.presentation.v1 body=enquire face=think -->\n", "Checking.", "toolUse");
  accept(projector, root("A"), 1, 5, { type: "tool_execution_start", privateToolArguments: "never" });
  const second = run(6, "<!-- harness.presentation.v1 body=happy face=happy -->\n", "Done.", "stop");
...
test("invalid, missing, and oversized reserved markers default without leakage", () => {
  for (const [id, streamed, final, expected] of [
    ["invalid", "<!-- harness.presentation.v1 body=private_asset_01 face=smile -->\nSafe.", "<!-- harness.presentation.v1 body=private_asset_01 face=smile -->\nSafe.", "Safe."],
    ["missing", "Ordinary prose.", "Ordinary prose.", "Ordinary prose."],
    ["oversized", `<!-- harness.presentation.v1 ${"x".repeat(300)}\nTail.`, `<!-- harness.presentation.v1 ${"x".repeat(300)}\nTail.`, "Tail."],
  ]) {
    ...
    assert.equal(terminal[0].event.text, expected); assert.doesNotMatch(JSON.stringify([...frames, ...terminal]), /private_asset|harness\.presentation\.v1 body=|x{20}/);
  }
});
...
test("terminal presentation mismatch fails the message and turn closed", () => {
  ...
  accept(projector, root("A"), 1, 2, { type: "message_update", assistantMessageEvent: { type: "text_delta", contentIndex: 0,
    delta: "<!-- harness.presentation.v1 body=happy face=smile -->\nStreamed." } });
  const terminal = accept(projector, root("A"), 1, 3, { type: "message_end", message: { role: "assistant", stopReason: "stop",
    content: [{ type: "text", text: "<!-- harness.presentation.v1 body=sad face=shame -->\nStreamed." }] } });
  ...
  assert.doesNotMatch(JSON.stringify(terminal), /body=sad|body=happy|harness\.presentation/);
});


test("root live and terminal projections scrub hostile reserved lines at every stream split", () => {
  const cue = "<!-- harness.presentation.v1 body=happy face=smile -->\n";
  const source = `${cue}Before.\n  <!-- harness.presentation.v1 body=sad face=shame -->\n` +
    "<!-- harness.presentation.v1 face=think body=talk-02 -->\n" +
    "<!-- harness.presentation.v1 body=yandere face=shy -->\n" +
    `<!-- harness.presentation.v1 ${"q".repeat(400)}\nAfter.`;
  ...
  for (let split = 0; split <= source.length; split += 1) {
    ...
    assert.deepEqual(emitted[0].event.presentation, { schema: "harness.presentation.v1", body: "happy", face: "smile" });
    ...
    assert.doesNotMatch(JSON.stringify([...emitted, ...terminal]), /body=sad|body=yandere|q{20}|harness\.presentation\.v1 body=/);
  }
});
