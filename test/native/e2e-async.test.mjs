...
function respond(response, id, { calls = [], answer = id, usage = { input_tokens: 10, output_tokens: 2, total_tokens: 12 } } = {}) {
...
export default function customCompactionFixture(pi) {
  pi.on("session_before_compact", async event => {
    ...
    ${customCompaction === "held" ? 'await fetch(process.env.HARNESS_FAKE_NATIVE_BASE_URL + "/gate/summary", { signal: event.signal }).then(response => response.text());' : ""}
    return { compaction };
  });
}
`);
...
test("authenticated family message during compaction waits for the summary and then continues once", { timeout: 120_000 }, async t => {
  const familyRequest = deferred(); let receiverId;
  const f = await fixture(t, body => {
    const last = body.input.at(-1);
    if (last?.role === "user" && text(last).includes("LONG_SUMMARY_CONTEXT")) return { answer: "L".repeat(60_000) };
    if (last?.role === "user" && text(last).includes("START_AUTOMATIC_SUMMARY")) return { answer: "BEFORE_SUMMARY",
      usage: { input_tokens: 30000, output_tokens: 2, total_tokens: 30002 } };
    if (last?.role === "user" && text(last).includes("SEND_DURING_SUMMARY")) {
      return { calls: [{ id: "send-during-summary", code: `await agent_message.send(${JSON.stringify(receiverId)}, "FAMILY_DURING_SUMMARY")` }] };
    }
    if (last?.role === "user" && text(last).includes("FAMILY_DURING_SUMMARY")) {
      familyRequest.resolve(body); return { answer: "FAMILY_AFTER_SUMMARY" };
    }
    return { answer: "SENDER_SETTLED" };
  }, { customCompaction: "held" });
  receiverId = f.sessionId;
  await submitAndSettle(f, "LONG_SUMMARY_CONTEXT_ONE");
  await submitAndSettle(f, "LONG_SUMMARY_CONTEXT_TWO");
  await f.submit("START_AUTOMATIC_SUMMARY");
  await bounded(f.gate("summary").started.promise, "held summary hook");
  ...
  await f.client.request("subscribe_session", { selector: sender.sessionId });
  const since = f.log.frames.length;
  await f.client.request("submit_input", { sessionId: sender.sessionId, message: "SEND_DURING_SUMMARY", behavior: "auto" });
  await f.log.waitFrame(frame => frame.event === "actor_event" && frame.data.sessionId === sender.sessionId
    && frame.data.event.type === "agent_settled", "sender settled while summary held", since);
  let family;
  const deliveryDeadline = Date.now() + 10_000;
  do {
    family = f.supervisor.store.listMessages().find(message => message.body === "FAMILY_DURING_SUMMARY");
    if (family?.state === "delivered") break;
    await new Promise(resolve => setTimeout(resolve, 20));
  } while (Date.now() < deliveryDeadline);
  assert(family); assert.equal(family.state, "delivered", "authenticated message reached the receiver");
  assert.equal((await f.state()).isCompacting, true);
  assert.deepEqual(await f.entries(), before, "incoming bookkeeping must not invalidate compaction");
  f.gate("summary").release.resolve();
  await f.log.wait(event => event.type === "compaction_end" && !event.errorMessage && !event.aborted, "summary completed");
  await bounded(familyRequest.promise, "family message consumed after summary");
  await f.log.waitFrame(frame => frame.event === "actor_event" && frame.data.sessionId === receiverId
    && frame.data.event.type === "agent_settled", "receiver resumed after summary", since);
  const entries = await f.entries();
  const compactionIndex = entries.findIndex(entry => entry.type === "compaction");
  assert(compactionIndex >= 0); assert.equal(entries.filter(entry => entry.type === "compaction").length, 1);
  const history = entries.filter(entry => entry.customType === "persistent-harness.agent-message-v1" && entry.data.messageId === family.messageId);
  assert.equal(history.length, 1); assert(entries.indexOf(history[0]) > compactionIndex);
  assert.equal(entries.filter(entry => entry.type === "custom_message" && entry.details?.messageId === family.messageId).length, 1);
  assert.equal(f.supervisor.store.getMessage(family.messageId).state, "acknowledged");
  assert.equal(entries.filter(entry => entry.message?.stopReason === "error").length, 0);
});
