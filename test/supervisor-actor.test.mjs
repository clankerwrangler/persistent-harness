...
test("concurrent root image and visible-message APIs preserve root presentation sanitization while child text stays unchanged", async (t) => {
  ...
  const source = "<!-- harness.presentation.v1 body=happy face=smile -->\nVisible.\n  <!-- harness.presentation malformed -->\nTail.";
  ...
  assert.doesNotMatch(JSON.stringify(rootVisible), /harness\.presentation|body=happy/);
...
test("supervisor projects native Pi progress globally and clears it on settle", async (t) => {
  ...
  const presentationMarker = "<!-- harness.presentation.v1 body=happy face=smile -->\n";
  ...
  assert.doesNotMatch(JSON.stringify(actorEvents), /private reasoning body|private tool arguments|private complete thinking|secret|harness\.presentation|body=happy/);
  ...
  assert.doesNotMatch(JSON.stringify(rootOutputEvents), /private reasoning body|private tool arguments|private complete thinking|secret|Testing durable progress replay|harness\.presentation\.v1 body=/);
  ...
  assert.doesNotMatch(JSON.stringify(replay.events), /private reasoning body|private tool arguments|private complete thinking|secret|harness\.presentation|body=happy/);
...
test("first assistant message starts a title while tool work stays active and leaves explicit names alone", async (t) => {
  ...
  const named = await client.request("create_root", {
    cwd: root, repositoryRoot: null, name: "Example 3D Avatar", provider: null, model: null, thinkingLevel: null,
  });
  await eventually(() => actors.get(named.admission.sessionId)?.isRunning);
  await client.request("submit_input", {
    sessionId: named.admission.sessionId,
    message: "I want this user title to stay put.",
    behavior: "auto",
  });
  actors.get(named.admission.sessionId).emit("event", { type: "agent_settled" });
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(supervisor.store.getSession(named.admission.sessionId).name, "Example 3D Avatar");
  ...
  await client.request("rename_session", { sessionId: pending.admission.sessionId, name: "user Chosen Name" });
  releaseTitle("Generated But Superseded");
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(supervisor.store.getSession(pending.admission.sessionId).name, "user Chosen Name");
