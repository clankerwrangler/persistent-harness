...
for (const rejection of ["previous_response_not_found", "websocket_connection_limit_reached"]) test(`WebSocket explicit ${rejection}: reconnect full history without duplicate call`, { timeout: 10000 }, async (t) => {
  ...
  assert.equal(Object.hasOwn(result, "nativeProviderError"), false, "successful reconnect does not retain rejected-attempt diagnostics");
  assert.equal(f.connections(), 2); assert.equal(f.requests.length, 3);
...
test("explicit rejection retry reports each retired attempt with a distinct ID", { timeout: 10000 }, async (t) => {
  ...
  assert.equal(Object.hasOwn(result, "nativeProviderError"), false);
});
...
test("provider error diagnostics retain only fixed code/category, never raw strings or payloads", async (t) => {
  const privateText = "PRIVATE_PROVIDER_DIAGNOSTIC";
  const known = [
    ["context_length_exceeded", "context"], ["invalid_request_error", "request"],
    ["invalid_value", "request"], ["missing_required_parameter", "request"],
    ["unsupported_parameter", "request"], ["unknown_parameter", "request"],
    ["invalid_api_key", "authentication"], ["authentication_error", "authentication"],
    ["permission_denied", "permission"], ["model_not_found", "not_found"],
    ["rate_limit_exceeded", "rate_limit"], ["insufficient_quota", "quota"],
    ["server_error", "server"], ["internal_error", "server"], ["service_unavailable", "server"],
    ["previous_response_not_found", "transport"], ["websocket_connection_limit_reached", "transport"],
  ];
  const cases = [
    ...known.map(([code, category]) => ({ code, type: privateText, expectedCode: code, category })),
    ...[privateText, "__proto__", "constructor", 17, false, {}, [], null, undefined].map(code =>
      ({ code, type: privateText, expectedCode: "unknown", category: "unknown" })),
    ...[["invalid_request_error", "request"], ["authentication_error", "authentication"],
      ["permission_error", "permission"], ["not_found_error", "not_found"],
      ["rate_limit_error", "rate_limit"], ["server_error", "server"], ["api_error", "server"]]
      .map(([type, category]) => ({ code: privateText, type, expectedCode: "unknown", category })),
    ...[null, 42, {}, [], "constructor"].map(type => ({ code: privateText, type, expectedCode: "unknown", category: "unknown" })),
    { code: "insufficient_quota", type: "authentication_error", expectedCode: "insufficient_quota", category: "quota" },
  ];
  for (const source of ["top", "nested", "terminal"]) for (const fixture of cases) {
    const error = { code: fixture.code, type: fixture.type, message: privateText, param: privateText,
      headers: { authorization: privateText }, payload: { privateText } };
    const raw = source === "terminal"
      ? { type: "response.failed", response: { ...complete().response, status: "failed", error } }
      : source === "nested" ? { type: "error", error }
        : { type: "error", code: fixture.code, error: { ...error, code: undefined }, message: privateText };
    let requests = 0;
    const a = await adapter(t, { fetch: async () => { requests++; return response([created(), raw]); } });
    const result = await collect(a.stream(model, context, { transport: "sse" }));
    assert.equal(result.message.stopReason, "error");
    assert.equal(result.message.errorMessage, fixture.code === "context_length_exceeded"
      ? "context_length_exceeded" : source === "terminal" ? "provider_response_failed" : "provider_error");
    assert.deepEqual(result.message.nativeProviderError, { version: 1, code: fixture.expectedCode, category: fixture.category });
    assert.ok(Object.isFrozen(result.message.nativeProviderError));
    assert.equal(JSON.stringify(result).includes(privateText), false);
    assert.equal(requests, 1, "auxiliary diagnostics never trigger retries");
  }
});

test("raw provider top-level code precedence and private hook errors keep existing policy", async (t) => {
  const a = await adapter(t, { fetch: async () => response([{ type: "error", code: "PRIVATE_TOP_CODE",
    error: { code: "context_length_exceeded", type: "invalid_request_error", message: "PRIVATE_PAYLOAD" } }]) });
  const result = await a.stream(model, context, { transport: "sse" }).result();
  assert.equal(result.errorMessage, "provider_error");
  assert.deepEqual(result.nativeProviderError, { version: 1, code: "unknown", category: "request" });
  assert.equal(JSON.stringify(result).includes("PRIVATE_"), false);
  const hook = await a.stream(model, context, { transport: "sse", onResponse() {
    throw Object.assign(new Error("PRIVATE_PAYLOAD"), { code: "context_length_exceeded" });
  } }).result();
  assert.equal(hook.errorMessage, "provider_request_failed");
  assert.equal(Object.hasOwn(hook, "nativeProviderError"), false, "only provider frames authorize diagnostics");
});


for (const ending of ["unknown", "provider_error"]) test(`rejected retry metadata does not contaminate final ${ending}`, async (t) => {
  const f = await websocketFixture(t, ({ socket, count }) => {
    if (count === 1) socket.send(JSON.stringify({ type: "error", code: "previous_response_not_found" }));
    else {
      send(socket, [created("resp_retry")]);
      if (ending === "unknown") socket.close();
      else socket.send(JSON.stringify({ type: "error", error: { code: "PRIVATE_RETRY_CODE", type: "PRIVATE_RETRY_TYPE" } }));
    }
  });
  const result = await f.a.stream(f.selected, context, { transport: "auto", sessionId: "session" }).result();
  assert.equal(result.nativeTransport.attempts.length, 2);
  assert.equal(result.errorMessage, ending === "unknown" ? "websocket_closed" : "provider_error");
  if (ending === "unknown") assert.equal(Object.hasOwn(result, "nativeProviderError"), false);
  else assert.deepEqual(result.nativeProviderError, { version: 1, code: "unknown", category: "unknown" });
  assert.equal(JSON.stringify(result).includes("PRIVATE_RETRY"), false);
  assert.equal(f.requests.length, 2);
});

test("adapter-produced interrupted ordinary suffix is excluded from both next-request projections without losing admitted prefix/result", async (t) => {
  const requests = [], entries = [], executed = [];
  const add = message => {
    const entry = { type: "message", id: `fixture_entry_${entries.length}`, parentId: entries.at(-1)?.id ?? null,
      timestamp: new Date(0).toISOString(), message: copied(message) };
    entries.push(entry); return entry;
  };
  add(context.messages[0]);
  const waitItem = callItem({ id: "fc_wait_incomplete", call_id: "call_wait_incomplete", name: "wait_for_ipython", arguments: "", async: undefined });
  const initialFrames = [created(), { type: "response.output_item.added", output_index: 0, item: { ...callItem(), arguments: "" } },
    doneCall(), { type: "response.output_item.added", output_index: 1, item: waitItem }];
  const a = await adapter(t, { fetch: async (_url, init) => {
    requests.push(JSON.parse(init.body));
    return response(requests.length === 1 ? initialFrames : [created("resp_next"), complete("resp_next")]);
  } });
  const tools = [tool, { name: "wait_for_ipython", description: "barrier", parameters: { type: "object", properties: {} } }];
  const planner = createActorStreamPlanner({ message: { role: "assistant", id: "fixture_segment_0", api: model.api,
    provider: model.provider, model: model.id, content: [], timestamp: 1, stopReason: "pending",
    usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0,
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } } },
    createMessageId: ({ segmentIndex }) => `fixture_segment_${segmentIndex}`, createTextId: ({ sourceIndex }) => `fixture_text_${sourceIndex}` });
  const commit = plan => {
    const prepared = planner.prepareCommit({ planId: plan.planId, message: plan.message });
    const entry = add(prepared.message);
    return planner.acknowledge({ planId: prepared.planId, entryId: entry.id, message: entry.message });
  };
  const firstStream = a.stream(model, { ...context, tools }, { transport: "sse" });
  for await (const event of firstStream) {
    const planned = planner.consume(event, a.nativeCompletion(event));
    if (planned.prefix) {
      const admitted = commit(planned.prefix).calls;
      assert.equal(admitted.length, 1); assert.equal(admitted[0].native, true);
      // The fixture's sole execution/result writer runs only acknowledged calls.
      executed.push(admitted[0].call.id);
      add({ role: "toolResult", toolCallId: admitted[0].call.id, toolName: "ipython", isError: false, timestamp: 2,
        content: [{ type: "text", text: "ACTUAL_PREFIX_RESULT" }], details: { checkpoint: "matching_fixture_checkpoint" } });
    }
    if (planned.final) assert.deepEqual(commit(planned.final).calls, [], "incomplete ordinary suffix was never admitted");
  }
  const first = await firstStream.result(), canonicalBytes = JSON.stringify(entries), failed = entries.at(-1).message;
  assert.equal(first.errorMessage, "provider_stream_incomplete");
  assert.equal(failed.stopReason, "error");
  assert.deepEqual(failed.nativeTransport, first.nativeTransport, "canonical remainder keeps actual adapter report");
  assert.deepEqual(failed.nativeTransport.observedCalls, [
    { itemId: "fc_original", callId: "call_original", complete: true, native: true },
    { itemId: "fc_wait_incomplete", callId: "call_wait_incomplete", complete: false, native: false },
  ]);
  assert.deepEqual(executed, ["call_original|fc_original"]);
  assert.equal(failed.content.length, 1);
  assert.equal(failed.content[0].id, "call_wait_incomplete|fc_wait_incomplete");
  ...
  for (const mode of ["native", "ordinary"]) {
    ...
    assert.equal(projected.diagnostics.filter(d => d.code === "UNADMITTED_CALL_OMITTED").length, 1);
    assert.deepEqual(projected.messages.find(m => m.id === failed.id), { ...failed, content: [] });
    const next = await a.stream(model, { ...context, tools, messages: convertToLlm(projected.messages) }, { transport: "sse" }).result();
    assert.equal(next.stopReason, "stop");
    const wire = requests.at(-1).input;
    assert.deepEqual(wire.filter(item => item.type === "function_call"), [{ type: "function_call", call_id: "call_original",
      id: "fc_original", name: "ipython", arguments: '{"code":"print(1)"}', async: true }]);
    assert.deepEqual(wire.filter(item => item.type === "function_call_output"), [{ type: "function_call_output",
      call_id: "call_original", output: "ACTUAL_PREFIX_RESULT" }]);
    assert.equal(JSON.stringify(wire).includes("wait_incomplete"), false);
