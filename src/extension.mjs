...
export default function persistentHarnessExtension(pi, owner = null) {
  ...
  async function appendHistoryEntry(sessionManager, type, data) {
    return owner?.appendHistoryEntry
      ? owner.appendHistoryEntry(type, data)
  ...
  async function ensureOutgoingAgentMessage(message) {
    ...
    if (entry) {
      ...
      const entryId = await appendHistoryEntry(sessionManager, AGENT_MESSAGE_ENTRY_TYPE, expected);
  ...
  async function ensureChildCreation(message) {
    ...
    if (entry) {
      ...
      const entryId = await appendHistoryEntry(sessionManager, CHILD_CREATION_ENTRY_TYPE, expected);
      entry = sessionManager.getEntry(entryId);
    }
  ...
  async function persistProgressHeading(summary) {
    ...
    const entryId = await appendHistoryEntry(sessionManager, PROGRESS_ENTRY_TYPE, { summary });
  ...
  async function flushUsageEntries() {
    ...
    for (const [entryId, usage] of [...pendingUsageEntries]) {
      ...
      try {
        ...
        await appendHistoryEntry(activeContext.sessionManager, USAGE_ENTRY_TYPE, { entryId });
