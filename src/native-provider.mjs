...
// Auxiliary diagnostics are exact fixed tokens, never raw provider text, IDs,
// parameters, or payloads. This does not participate in retry classification.
const providerErrorCodes = new Map([
...
const providerErrorTypes = new Map([
...
function safeProviderError(code, type) {
  return Object.freeze({ version: 1, code: providerErrorCodes.has(code) ? code : "unknown",
    category: providerErrorCodes.get(code) ?? providerErrorTypes.get(type) ?? "unknown" });
}
...
export function createNativeProviderAdapter({ api, responsesApi, modelRuntime, transportOptions = {} }) {
  ...
  function stream(model, context, options = {}) {
    ...
    let attempt, nativeProviderError, providerTools = false;
    const beginAttempt = (transport) => {
      nativeProviderError = undefined;
    ...
    (async () => {
      ...
      try {
        ...
        const observed = async function* (wire, retired) {
          for await (const raw of wire) {
            ...
            if (raw.type === "error") {
              ...
              nativeProviderError = safeProviderError(code, raw.error?.type);
            ...
            if (terminalTypes.has(raw.type)) {
              ...
              if (raw.type === "response.failed") {
                nativeProviderError = safeProviderError(raw.response?.error?.code, raw.response?.error?.type);
                fail(raw.response?.error?.code === "context_length_exceeded" ? "context_length_exceeded" : "provider_response_failed");
              }
        ...
        if (final?.type === "error" && nativeProviderError) output.nativeProviderError = nativeProviderError;
